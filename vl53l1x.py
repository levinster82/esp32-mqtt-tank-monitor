...
MicroPython driver for STMicroelectronics VL53L1X laser ranging sensor
...
# Default sensor configuration from ST datasheet
# This 91-byte configuration array sets up the sensor registers
# for standard ranging mode with reasonable defaults
...
class VL53L1X:
    ...
    This driver provides basic distance measurement functionality for the
    VL53L1X laser ranging sensor over I2C.
    """
    ...
    def __init__(self, i2c, address=0x29):
        ...
        Raises:
            RuntimeError: If sensor model ID doesn't match expected value (0xEACC)
        """
        self.i2c = i2c
        ...
        # Write default configuration from ST datasheet
        self.i2c.writeto_mem(self.address, 0x2D, VL53L1X_DEFAULT_CONFIGURATION, addrsize=16)

        # Configure timing budget (from ST API initialization)
        # Assumes MM1 and MM2 disabled
        self.writeReg16Bit(0x001E, self.readReg16Bit(0x0022) * 4)
        time.sleep_ms(200)  # Allow configuration to settle
    ...
    def read(self):
        ...
        Returns:
            int: Distance in millimeters, or invalid reading if sensor error

        Note:
            This is a simple implementation that returns the raw distance.
            For production use, you may want to check range_status for errors.
        """
        # Read 17 bytes from RESULT__RANGE_STATUS register
